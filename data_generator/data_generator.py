...
class SampleSaver:
    def __init__(self, labels_path):
        ...
        with open(self.labels_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['id', 'file1', 'file2', 'letter', 'is_same_frame'])
...
def add_noise(img: Image.Image):
    ...
    for _ in range(random.randint(3, 5)):
        ...
        # Losowy punkt na górnym i dolnym boku
...
def crop_sprite(img):
    width, height = img.size
    num_cols = random.randint(8, 15)
    side = random.choice(['left', 'right'])

    # Upewnij się, że obrazek ma kanał alfa
    if img.mode != 'RGBA':
        img = img.convert('RGBA')

    pixels = img.load()
    for x in range(num_cols):
        col = x if side == 'left' else width - 1 - x
        for y in range(height):
            # Ustaw piksel na przezroczysty
...
def worker_generate_sample(args):
    ...
    for pair_idx, ((i, j), letter) in enumerate(zip(segment_pairs, letters)):
        ...
        outfit_img2 = outfit_images[outfit_path2]
        out1 = pick_outfit_frame(outfit_img, frame)
        out2 = pick_outfit_frame(outfit_img2, frame2)
        out1 = crop_sprite(out1)
        out2 = crop_sprite(out2)
        offset_x1 = random.randint(1, 10)
        offset_y1 = random.randint(2, 12)
        offset_x2 = random.randint(1, 10)
        offset_y2 = random.randint(2, 12)
        segments[i].paste(out1, (offset_x1, offset_y1), out1 if out1.mode == 'RGBA' else None)
        segments[j].paste(out2, (offset_x2, offset_y2), out2 if out2.mode == 'RGBA' else None)
        file1 = f'seg_{local_id}_1.jpg'
        file2 = f'seg_{local_id}_2.jpg'
        img_saver.save_async(segments[i].copy(), file1)
        img_saver.save_async(segments[j].copy(), file2)
        samples.append([local_id, file1, file2, letter, is_same])
        local_id += 1
    return samples
...
def generate_sample():
    ...
    out_dir = os.path.join('data', 'captcha', 'dataset', 'images')
    labels_path = os.path.join('data', 'captcha', 'dataset', 'labels.csv')
